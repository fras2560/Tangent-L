 ...
 */
public class AdjustedBM25Similarity extends Similarity {
   ...
   */
  public AdjustedBM25Similarity(float k1, float b) {
  ...
  private class BM25DocScorer extends SimScorer {
    ...
    BM25DocScorer(BM25Stats stats, NumericDocValues norms) throws IOException {
      this.stats = stats;
      this.weightValue = stats.weight * (AdjustedBM25Similarity.this.k1 + 1);
    ...
    public float score(int doc, float freq) {
      ...
      final float norm =
          this.norms == null
              ? AdjustedBM25Similarity.this.k1
              : this.cache[(byte) this.norms.get(doc) & 0xFF];
      return this.weightValue * 1 / (1 + norm);
    }

    @Override
    public Explanation explain(int doc, Explanation freq) {
      return AdjustedBM25Similarity.this.explainScore(doc, freq, this.stats, this.norms);
    }

    @Override
    public float computeSlopFactor(int distance) {
      return AdjustedBM25Similarity.this.sloppyFreq(distance);
    }

    @Override
    public float computePayloadFactor(int doc, int start, int end, BytesRef payload) {
      return AdjustedBM25Similarity.this.scorePayload(doc, start, end, payload);
    }
  }

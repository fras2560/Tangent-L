...
public class EvaluateFeatures extends FindOptimal{
    ...
    public EvaluateFeatures(Path documents,
                            ...
                            Path queries,
                            Path results,
                            boolean documentLevel) throws XPathExpressionException,
                                              IOException,
    ...
    public double outputFeatureMeasure(Map<String, double[]> scoreLookup,
                                       List<String> copyFeatures,
                                       String feature,
                                       int measurement) throws IOException{
        // now check it versus all its input
        ConvertConfig config;
        double mrrPos, mrrNeg, delta;
     ...
     * Runs an experiment used to evaluate the effectiveness
     * Arguments:
     *  indexDirectory : the path to the index
     *  queries: a path to file with the queries
     *  results: a path to the results file to use
     *  documents: a path to the documents
     *  logFile: a path to the file to log to 
    ...
    public static void main(String[] args) throws IOException{
        ...
        try {
            ...
            output = Paths.get(cla.getPath(CommandLineArguments.RESULTSFILE).toString(), timeStamp + "-evaluateFeatures.txt");
            ...
            if(cla.getPath(CommandLineArguments.LOGFILE) != null){
                logFile = Paths.get(cla.getPath(CommandLineArguments.LOGFILE).toString(), timeStamp = "-evaluateFeatures.log");

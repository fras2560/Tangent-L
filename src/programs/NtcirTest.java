...
/**
 * A program to run the NTCIR test on an index given a set of queries and outputs the results to a
 * few different files.
 *
...
public class NtcirTest {
  ...
  public static void main(String[] args) throws Exception {
    final String usage =
        "Usage:\tjava programs.NTCIR_TEST [-index dir] [-queries file]"
            + " [-judgements file] [-resultsOutput file] [-queriesOutput file] [-logFile file]";
    ...
    Path queries =
        Paths.get(
            System.getProperty("user.dir"), "resources", "query", "NTCIR12-MathWiki-formula.xml");
    Path results =
        Paths.get(
            System.getProperty("user.dir"),
            "resources",
            "results",
            "NTCIR12-MathWiki-formula-dashes.dat");
    ...
    Path queryOutput =
        Paths.get(
            System.getProperty("user.dir"), "resources", "output", "arXiv", date + "-queries.txt");
    Path resultOutput =
        Paths.get(
            System.getProperty("user.dir"), "resources", "output", "arXiv", date + "-results.txt");
    ...
    try {
      ...
      final File queryText = queryOutput.toFile();
      queryText.createNewFile();
      final FileOutputStream qis = new FileOutputStream(queryText);
      final OutputStreamWriter qosw = new OutputStreamWriter(qis);
      queryWriter = new BufferedWriter(qosw);
      // write out the precisions results of each file
      final File resultsText = resultOutput.toFile();
      resultsText.createNewFile();
      final FileOutputStream ris = new FileOutputStream(resultsText);
      final OutputStreamWriter rosw = new OutputStreamWriter(ris);

 ...
 */
final class MathScorer extends BulkScorer {
  ...
  MathScorer(MathWeight weight, boolean disableCoord, int maxCoord, Collection<BulkScorer> scorers, int minShouldMatch, boolean needsScores) {
    ...
    for (BulkScorer scorer : scorers) {
      if (needsScores == false) {
        ...
        scorer = MathWeight.disableScoring(scorer);
      }
    ...
    if (coordFactors[1] == 1f) {
      ...
      singleClauseCollector = new FilterLeafCollector(collector) {
        ...
        public void setScorer(Scorer scorer) throws IOException {
          super.setScorer(new MathBooleanTopLevelScorer.BoostedScorer(scorer, coordFactors[1]));
        }
      };
    }

 ...
 */
final class MathWeight extends Weight {
  /** The Similarity implementation. */
  final Similarity similarity;
  final MathBooleanQuery query;
  ...
  MathWeight(MathBooleanQuery query, IndexSearcher searcher, boolean needsScores, boolean disableCoord) throws IOException {
    super(query);
    this.query = query;
  ...
  BulkScorer optionalBulkScorer(LeafReaderContext context) throws IOException {
    ...
    if (optional.size() == 1) {
      ...
      if (!disableCoord && maxCoord > 1) {
        return new MathBooleanTopLevelScorer.BoostedBulkScorer(opt, coord(1, maxCoord));
      } else {
        return opt;
      }
    }

    return new MathScorer(this, disableCoord, maxCoord, optional, Math.max(1, query.getMinimumNumberShouldMatch()), needsScores);
  }
  ...
  public Scorer scorer(LeafReaderContext context) throws IOException {
    ...
    if (disableCoord) {
      ...
      if (minShouldMatch > 0) {
        ...
        return new MathBooleanTopLevelScorer.ReqSingleOptScorer(req, opt, coordReq, coordBoth);
      }
    } else {
      if (minShouldMatch > 0) {
        return new MathBooleanTopLevelScorer.CoordinatingConjunctionScorer(this, coords, req, requiredScoring.size(), opt);
      } else {
        return new MathBooleanTopLevelScorer.ReqMultiOptScorer(req, opt, requiredScoring.size(), coords); 
      }
    }
  }
  ...
  private Scorer req(List<Scorer> required, List<Scorer> requiredScoring, boolean disableCoord) {
    if (required.size() == 1) {
      ...
      return new MathBooleanTopLevelScorer.BoostedScorer(req, boost);
    } else {
  ...
  private Scorer opt(List<Scorer> optional, int minShouldMatch, boolean disableCoord) throws IOException {
    if (optional.size() == 1) {
      ...
      if (!disableCoord && maxCoord > 1) {
        return new MathBooleanTopLevelScorer.BoostedScorer(opt, coord(1, maxCoord));
      } else {
        return opt;
      }
    } else {

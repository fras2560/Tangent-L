...
public class FileStatistics {
  private final List<Float> formulas;
  private float words;
    ...
    while ((character = is.read()) != -1) {
      if (Character.isWhitespace(character)) {
        if (token.startsWith("NUMBER SIGN")
            ...
            && !(token.equals(Constants.FORMULA_START_TAG)
                || token.equals(Constants.FORMULA_END_TAG))) {
  ...
  public float getFormulaCount() {
    ...
    for (final Float formula : this.formulas) {
      total += formula;
    }
  ...
  public float averageFormulaSize() {
    ...
    final Float average = total / new Float(this.formulas.size());

...
public class CommandLineArguments {
  ...
  private final Map<String, Path> configLookup;
  ...
  public static Map<String, Path> loadConfig() throws IOException {
    final Path config = Paths.get(System.getProperty("user.dir"), "config.txt");
    final BufferedReader fileReader = new BufferedReader(new FileReader(config.toFile()));
    String line;
    String attribute;
    Path value;
    String[] parts;
    final Map<String, Path> configLookup = new HashMap<String, Path>();
   ...
   * @throws IOException - issue while dealing with a file
   */
  public CommandLineArguments(String[] args, List<String> required)
    ...
    for (final String check : required) {
      if (check.equals(CommandLineArguments.INDEXDIRECTORY) && this.indexPath == null) {
        ...
        if (this.indexPath == null) {
          throw new CommandLineException(
              "Missing Required IndexDirectory and not listed in Config");
        }
        ...
        if (this.resultsPath == null) {
          throw new CommandLineException(
              "Missing Required ResultsDirectory and not listed in Config");
        }
      } else if (check.equals(CommandLineArguments.QUERIESOUTPUTFILE)
          && this.queriesOutputPath == null) {
        this.queriesOutputPath =
            this.pathOrLookup(this.queriesOutputPath, CommandLineArguments.QUERIESOUTPUTFILE);
        if (this.queriesOutputPath == null) {
          throw new CommandLineException(
              "Missing Required Queries Output File and not listed in Config");
        }
      } else if (check.equals(CommandLineArguments.DOCUMENTSDIRECTORY)
          && this.documentsPath == null) {
        this.documentsPath =
            this.pathOrLookup(this.documentsPath, CommandLineArguments.DOCUMENTSDIRECTORY);
        if (this.documentsPath == null) {
          throw new CommandLineException(
              "Missing Required Documents Folder and not listed in Config");
        }
        ...
        this.judgementsPath =
            this.pathOrLookup(this.judgementsPath, CommandLineArguments.JUDGEMENTS);
        if (this.judgementsPath == null) {
          throw new CommandLineException(
              "Missing Required Judgements File and not listed in Config");
        }
      }
    }
  }
